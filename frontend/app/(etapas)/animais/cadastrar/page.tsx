...
const FORM_INITIAL: FormData = {
  ...
  setor: "", canil: "", cor: "", temperamento: "",
  vacinacao: "", dataVacinacao: "", outros: "",
};
...
export default function CadastroPet() {
  ...
  const handleChange = (
    ...
    setFormData((prev) =>
      name === "setor" ? { ...prev, setor: value, canil: "" } : { ...prev, [name]: value }
    );
  };
    ...
    <div className="relative min-h-screen bg-white overflow-x-hidden flex flex-col md:ml-56 pb-20 md:pb-0">
      ...
      <main className="relative z-10 px-6 py-8 flex-1 w-full max-w-md mx-auto">
        ...
        <div className="flex justify-center mb-8">
          <label className="relative w-32 h-32 rounded-full bg-gray-200 border-[5px] border-[#2DB38B] flex items-center justify-center shadow-sm cursor-pointer overflow-hidden">
            {photoPreview ? (
              ...
              <img src={photoPreview} alt="Preview do pet" className="w-full h-full object-cover" />
            ) : (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <InputGroup label="Nome do animal"      name="nome"          placeholder="Ex: Robson"        value={formData.nome}          onChange={handleChange} />
          <SelectGroup label="Tipo do animal"     name="tipo"          options={["Cachorro", "Gato"]}  value={formData.tipo}          onChange={handleChange} />
          <InputGroup label="Idade (aproximada)"  name="idade"         placeholder="Ex: 3 anos"        value={formData.idade}         onChange={handleChange} />
          <SelectGroup label="Sexo"               name="sexo"          options={["Masculino", "Feminino"]} value={formData.sexo}      onChange={handleChange} />

          <div className="grid grid-cols-2 gap-3">
            <SelectGroup label="Setor" name="setor" options={SETORES}       value={formData.setor} onChange={handleChange} />
            <SelectGroup label="Canil" name="canil" options={canilOptions}  value={formData.canil} onChange={handleChange} disabled={!formData.setor} />
          </div>

          <InputGroup label="Coloração do pelo"  name="cor"           placeholder="Ex: Marrom"        value={formData.cor}           onChange={handleChange} />
          <InputGroup label="Temperamento"        name="temperamento"  placeholder="Ex: Dócil"         value={formData.temperamento}  onChange={handleChange} />
          <SelectGroup label="Vacinação"          name="vacinacao"     options={["Completa", "Incompleta", "Não vacinado"]} value={formData.vacinacao} onChange={handleChange} />
          <InputGroup label="Data de vacinação"   name="dataVacinacao" type="date"                     value={formData.dataVacinacao} onChange={handleChange} />
